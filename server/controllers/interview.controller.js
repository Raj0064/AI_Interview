...
export const generateQuestions = async (req, res) => {
  try {
    ...
    if (!role || !experience || !mode) {
      res
        .status(400)
        .json({ message: `Role Experience and Mdoe are required` });
    }
    ...
    if (user.credits < 50) {
      ...
      return res
        .status(400)
        .json({ message: "Not enough credits, Min 50 required." });
    }
...
`,
    ...
    return res
      .status(500)
      .json({ message: `Question generation Error , ${error}` });
  }
};
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    if (!answer) {
      question.score = 0;
      question.feedback = "You didnt submit an answer";
      question.answer = "";
